...
OneZip.prototype._extract = function (from) {
    ...
    yauzl.open(from, options, function (error, zipfile) {
        ...
        zipfile.on('entry', function (entry) {
            ...
            var name = path.join(_this9._to, fileName);

            if (/\/$/.test(fileName)) return mkdirp(name, fn);

            zipfile.openReadStream(entry, _this9._onOpenReadStream(function (readStream) {
                _this9._writeFile(name, readStream, fn);
            }));
        });
